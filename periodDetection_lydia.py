...
def getRedline(input):
    ...
    imgCopy = input.copy()
    test = np.zeros(imgCopy.shape, dtype=np.float32)
    imgCopy.fill(0)
    angles = []
    liney = []
    linesCopy = []
        ...
        for line in liney:
            for otherLine in range(len(lines)):
                if otherLine in liney:
                    if abs(lines[line][0][0] - lines[otherLine][0][0]) < 15.0 and abs(
                            lines[line][0][0] - lines[otherLine][0][0]) > 0:
    ...
    return imgCopy, lines_y

def getDistanceBetweenStrings(lines):
    lines.sort()
    distance = []
    ...
    print(lines)
    print(distance)
    average_distance = sum(distance)/(len(lines)-1)
...
def getTemplates(input, redLine):
    ...
    for i in range(redLine.shape[0]):
        if string[i][centerLine] != 0:
            curBox = curRedline[i - int(boxRange * 1.2):i + int(boxRange * 1.2),
                     centerLine - boxRange * 2:centerLine + boxRange * 2]
            if curBox[curBox > 0].any():
                # print("curLine contains straight line")
                continue

            templates.append(input[i - int(boxRange * 1.2):i + int(boxRange * 1.2),
                             centerLine - boxRange * 2:centerLine + boxRange * 2])
...
def imageProcess(frame, redlineImg):
    ...
    redline, linesWithAngle = getRedline(redlineImg)
    cv2.imshow('HoughLine', redline)

    distance = getDistanceBetweenStrings(linesWithAngle)
